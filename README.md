...
### Headless server (no VSCode required)

The extension bundles a standalone ticket server — `dist/server.cjs` in the extension install — that serves the **same** loopback MCP HTTP API against the **same** `<workspace>/.vscode/dostuff/dostuff.db` and registers in the **same** instance registry. The agent skill (and any other MCP client) finds it exactly like a running extension, so ticket work no longer requires an open VSCode window — and future IDE integrations get a process to talk to instead of a codebase to port.

```sh
node ~/.vscode/extensions/<publisher>.dostuff-*/dist/server.cjs serve --workspace /path/to/repo
```

- `serve [--workspace PATH] [--port N] [--storage-dir PATH] [--takeover] [--no-sync] [--print-config]` — serve one workspace (default: cwd). Prints `{"port": N, ...}` on stdout when ready; logs to stderr; `Ctrl-C` unregisters and exits cleanly.
- `status` — list registered instances (workspace, port, pid, liveness).
- **Config parity**: the server resolves `flags > DOSTUFF_* env vars (DOSTUFF_MCP_PORT, DOSTUFF_STORAGE_DIR, DOSTUFF_ACTIVE_LANE_CAP, DOSTUFF_MCP_RECORD_LIMIT, DOSTUFF_MCP_INSTRUCTIONS) > the workspace's .vscode/settings.json dostuff.* keys > defaults` — so one workspace behaves identically under both hosts (same lane cap, record window, custom instructions; comments and trailing commas in settings.json are fine). `--print-config` shows every resolved value and where it came from.
- **Same rules**: agent write boundaries, lane caps, field caps, and human-only Complete/Closed are enforced in the shared core — there is no privileged headless path.
- **Single-writer gate**: `serve` refuses a workspace that a live process (VSCode window or another headless server) already serves — exit 3, naming the pid/port — because the DB is wholesale-rewrite and two writers clobber each other. `--takeover` overrides for zombie entries. The inverse (opening VSCode while a headless server runs) is the same two-window clobber class as before: enable **git ticket sync** and the two reconcile through the ref automatically (the headless server runs the same sync controller, honoring `dostuff.sync.*` from settings.json; `--no-sync` opts out).
- Close-request approvals remain human actions in the VSCode UI — a headless-filed `request_ticket_close`/`request_ticket_complete` parks in `pendingClose` until a human opens the board, same as an unattended window.
...
| `update_ticket_description` | `id`, `description` (≤10,000 chars), optional `note` (≤500), optional `expectedUpdatedAt` | Replaces the ticket's description (and appends one record entry). Allowed on Thinking + active-lane tickets; Complete and Closed are rejected. Only the description changes — title, priority, type, and verifyCriteria stay locked. Replace-shaped, so it supports the opt-in CAS guard: pass `expectedUpdatedAt` (the `updatedAt` from your read) and a ticket that changed since rejects the write, embedding the fresh description to merge into — instead of silently erasing a concurrent edit. |
...
| `update_ticket_draft` | `id`, optional `tags[]`, `links[]`, `tasks[]`, optional `expectedUpdatedAt` | Reshapes an untriaged draft's tags, links, and/or task list. **Thinking-only** — rejected once the ticket is triaged to an active lane (use the UI after that). Omit a field to leave it unchanged; pass `[]` to clear it. Unknown link targets are dropped. Supports the same `expectedUpdatedAt` CAS guard as `update_ticket_description` (stale token → rejection embedding the fresh tags/links/tasks), since it replaces whole lists. |
...
- **Claude Code agent skill** — `skills/dostuff-tickets/` covers the full MCP surface over bare curl, so Claude Code can drive the queue without registering the server as an MCP client (near-zero per-session context cost). Ships in the vsix: install with **DoStuff: Install Claude Code Agent Skill**, or as a plugin via `/plugin marketplace add wcole3/DoStuff`. The helper script discovers the per-workspace port from the instance registry and enforces the server's field caps locally before sending. Command-installed copies auto-refresh when the extension updates (version+hash marker; local edits are never overwritten silently). See "Claude Code Agent Skill" above.
- **Headless ticket server** — `dist/server.cjs` ships in the vsix: the same MCP HTTP API, DB, registry, write boundaries, and (opt-in) git sync, with no VSCode window. `node <ext-install>/dist/server.cjs serve --workspace /path/to/repo`; the agent skill discovers it unchanged. Reads `dostuff.*` from the workspace's `.vscode/settings.json` (JSONC-tolerant) with flag/env overrides, and refuses a workspace another live instance already serves. See "Headless server" above.
- **Safer concurrent agent writes.** Mutating MCP tool calls are now serialized per store (parallel subagents over curl previously raced read-modify-write and could drop record entries), overlapping DB flushes coalesce through an export queue instead of racing the final rename, and the two replace-shaped writes (`update_ticket_draft`, `update_ticket_description`) accept an opt-in `expectedUpdatedAt` compare-and-swap token — a stale write is rejected with the fresh state embedded instead of silently erasing a concurrent edit. Reads now expose `updatedAt` as the token.
