...
- **Schema changes are additive only.** Add new optional fields and new tables; never rename, retype, or repurpose an existing column/field. If a field's meaning must change, introduce a new one and migrate. (Precedent: `tags`, `links` (+ the `issue_links` table), and `attachments` were *added*; an older DB or JSON without them loads with empty defaults. `pendingClose` (+ the `issue_pending_close` table) added a 0-or-1 relation defaulting to `null`. Most recently sync added `guid`/`updatedAt` on issues, optional `tasks[].updatedAt`, and the `sync_tombstones` table — all with lazy/derived defaults.)
