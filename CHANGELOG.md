...
- Full-tab Kanban board with HTML5 drag-and-drop across five lanes: Thinking drawer → Planned → Working → Verification → Complete drawer.
- Active-lane cap of 6 open tickets per lane (Planned, Working, Verification); enforced in the board UI, the host's `applyIssueUpdate` chokepoint, and the MCP `update_ticket_status` tool.
...
- Active-lane cap (6 per Planned/Working/Verification) is now enforced on `importIssues` too; previously imports could exceed the cap silently.
